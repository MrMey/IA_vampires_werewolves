...
def heuristic(humans, allies, enemies):
    ...
    for human in humans:
        ...
        if (min_dist_en is None and min_dist_al is not None) or (min_dist_en is not None and min_dist_al is not None
            ...
            result += humans[human] / (max(2, min_dist_al))
        elif min_dist_en is not None:
            result -= humans[human] / (max(2, min_dist_en))
    for ally in allies:
        dmin = None
        enemy = None
        for en in enemies:
        ...
        if dmin is not None:
            if allies[ally] > 1.5 * enemies[enemy]:
                result += enemies[enemy] / (max(2, dmin))
            elif 1.5 * allies[ally] < enemies[enemy]:
                result -= allies[ally] / (max(2, dmin))
            else:
                p = allies[ally] / (2 * enemies[enemy])
                result += (p ** 2) * allies[ally] / (max(2, dmin)) - ((1 - p) ** 2) * enemies[enemy] / (max(2, dmin))
...
def get_next_move_alpha_beta(depth, grid):
    return alpha_beta_max(depth, grid.humans, grid.allies, grid.enemies, None, (grid.width, grid.height), True)


def alpha_beta_max(depth, humans, allies, enemies, value, dimensions, get_moves=False):
    if depth <= 0:
        ...
        while i < len(children) and (interval[0] is None or interval[1] is None or interval[0] < interval[1]):
            ...
            if child[4]:
                ...
                val = alpha_beta_min(depth - 1, child[0], child[1], child[2], interval[0], dimensions)
            if get_moves and (interval[0] is None or (val is not None and val > interval[0])):
                move = child[3]
            if interval[0] is None:
                interval = (val, interval[1])
            elif val is not None:
                interval = (max(val, interval[0]), interval[1])
            if get_moves:
                print(move, interval)
        if get_moves:
...
def alpha_beta_min(depth, humans, allies, enemies, value, dimensions):
    if depth <= 0:
        return heuristic(humans, allies, enemies)
    else:
        interval = (value, None)
        ...
        while i < len(children) and (interval[0] is None or interval[1] is None or interval[0] < interval[1]):
            ...
            if child[4]:
                ...
                val = alpha_beta_max(depth - 1, child[0], child[1], child[2], interval[1], dimensions)
            if interval[1] is None:
                interval = (interval[0], val)
            elif val is not None:
                interval = (interval[0], min(val, interval[1]))
        return interval[1]
...
top = time.time()
print(alpha_beta_max(DEPTH, humans, allies, enemies, None, (15,15), True))
print(time.time()-top)
